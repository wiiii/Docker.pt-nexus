...
class HdkylUploader(SpecialUploader):
    ...
    def _map_parameters(self) -> dict:
        ...
        if year_value:
            ...
            logger.info(f"HDKyl年份映射: {year_value}")
    ...
    def _extract_and_map_year(self) -> str:
        ...
        if not original_year:
            logger.warning("未找到年份参数")
            return ""
        
        # 提取前4位数字（年份）
        year_match = re.search(r'(\d{4})', original_year)
        if not year_match:
            logger.warning(f"无法从年份参数 '{original_year}' 中提取4位数字")
            return ""
        
        year_4digit = year_match.group(1)
        logger.info(f"提取到4位年份: {year_4digit}")
        ...
        if cut_version:
            ...
            logger.info(f"检测到cut_version，增强年份参数: {enhanced_year}")
    ...
    def _extract_cut_version(self, title_params: dict) -> str:
        ...
        if main_title:
            matches = re.findall(cut_version_pattern, main_title, re.IGNORECASE)
            if matches:
                # 返回第一个匹配的cut_version
                cut_version = matches[0]
                logger.info(f"从主标题中提取到cut_version: {cut_version}")
                return cut_version
        ...
        for key, value in title_params.items():
            if key != "主标题" and value:
                ...
                if matches:
                    cut_version = matches[0]
                    logger.info(f"从标题组件 '{key}' 中提取到cut_version: {cut_version}")
                    return cut_version
    ...
    def _map_year_to_value(self, year_text: str) -> str:
        ...
        logger.warning(f"年份 '{year_text}' 映射失败，使用默认值: {default_value}")
